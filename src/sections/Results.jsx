...
// Car data with proper image handling
const CARS = [
...
function daysBetweenInclusive(startISO, endISO) {
  const a = new Date(startISO), b = new Date(endISO || startISO);
...
function formatClock(isoDate, hhmm, plusMinutes) {
  try {
    ...
    const am = hr < 12 ? "am" : "pm"; hr = hr % 12 || 12;
...
const FALLBACK_IMAGES = {
  ...
  fortuner: "https://via.placeholder.com/112x80/6366F1/FFFFFF?text=Fortuner"
};
...
export default function Results({
  ...
  /* --- Determine package key (priority: searchParams.package / searchParams.packageId -> initialPackageId -> default 8x80) --- */
  const getCityPackageKey = () => {
    // possible values from Hero: "8h", "12h", "8x80", "12x120", "full" etc.
    const pkgRaw = (searchParams?.package || searchParams?.packageId || initialPackageId || "").toString().trim().toLowerCase();
    if (!pkgRaw) return "8x80";
    if (pkgRaw === "12x120" || pkgRaw === "12h" || pkgRaw === "12") return "12x120";
    if (pkgRaw === "8x80" || pkgRaw === "8h" || pkgRaw === "8") return "8x80";
    // fallbacks: if package contains 12 or 120
    if (pkgRaw.includes("12") || pkgRaw.includes("120")) return "12x120";
    return "8x80";
  };

  const cityPackKey = useMemo(getCityPackageKey, [searchParams?.package, searchParams?.packageId, initialPackageId]);
  ...
  const autoKm = useMemo(() => {
    ...
    if (service === "outstation") {
      ...
      if (searchParams?.tripType === "roundtrip") {
        const oneSide = kmFromSelections(searchParams?.pickup, searchParams?.drop);
  ...
  const arrivalClock = useMemo(
    () => formatClock(searchParams?.pickupDate, searchParams?.pickupTime, arrivalDelayMin),
    [searchParams, arrivalDelayMin]
  );

  const days = useMemo(() => {
    if (service === "outstation" && searchParams?.tripType === "roundtrip")
      return daysBetweenInclusive(searchParams?.pickupDate, searchParams?.returnDate);
    return 1;
  ...
  function priceCity(carId) {
    // now use cityPackKey derived from searchParams OR initialPackageId
    const cfg = PRICE.city[cityPackKey]?.[carId];
    if (!cfg) return null;
    // attach meta about which package used
    const meta = {
  ...
  const headerLine = useMemo(() => {
    if (service === "airport") {
      const left = airportMode === "drop"
        ? searchParams?.pickup?.label || "Pick-up"
        : searchParams?.airport?.label || "Airport";
      const right = airportMode === "drop"
        ? searchParams?.airport?.label || "Airport"
        : searchParams?.drop?.label || "Drop";
      return `From: ${left} · To: ${right}`;
    }
    if (service === "outstation")
      return `From: ${searchParams?.pickup?.label || "From"} · To: ${searchParams?.drop?.label || "To"} (${searchParams?.tripType === "roundtrip" ? "Round Trip" : "One Way"})`;
    if (service === "local") {
  ...
  const handleImageError = (e, carId) => { e.target.src = FALLBACK_IMAGES[carId]; };

  return (
    <section className="max-w-5xl mx-auto px-4 py-6">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h2 className="text-lg md:text-xl font-semibold">{headerLine}</h2>
          <p className="text-sm text-gray-600 mt-1">{PRICE.airport.note}</p>
        </div>
      ...
      <div className="mt-4 grid grid-cols-2 md:grid-cols-3 gap-4">
        <div className="p-3 rounded-lg border bg-white">
          <label className="text-xs text-gray-600">Estimated distance (km)</label>
          <input type="number" min="0" step="1" value={km ?? 0}
            onChange={(e) => setKm(Number(e.target.value) || 0)}
            className="mt-1 w-full p-2 border rounded" placeholder="Auto if possible, else enter km" />
          {autoKm !== null ? (
            <p className="text-[12px] text-gray-500 mt-1">Auto-estimated from map • You can edit</p>
          ) : (
            <p className="text-[12px] text-gray-500 mt-1">No map coords — please type approx road km</p>
          )}
        </div>

        <div className="p-3 rounded-lg border bg-white">
          <label className="text-xs text-gray-600">Arrival delay (minutes)</label>
          <input type="number" min="0" step="1" value={arrivalDelayMin}
            onChange={(e) => { setTouchedDelay(true); setArrivalDelayMin(Number(e.target.value) || 0); }}
            className="mt-1 w-full p-2 border rounded" />
          <div className="mt-1 text-[12px] text-gray-700">
            <span className="inline-block px-2 py-[2px] rounded bg-gray-100">≈ {prettyDuration(estimatedTravelMin)}</span>
            <button type="button" onClick={() => { setTouchedDelay(false); setArrivalDelayMin(estimatedTravelMin); }}
              className="ml-2 underline text-rose-600">Use this</button>
          </div>
          <p className="text-[12px] text-gray-500 mt-1">Arrival clock time: {arrivalClock || "—"}</p>
        </div>

        <div className="p-3 rounded-lg border bg-white col-span-2 md:col-span-1">
          <label className="text-xs text-gray-600">Notes</label>
          <div className="text-[12px] mt-2 text-gray-700">{subNote}</div>
        </div>
      </div>
      ...
      <div className="mt-6 space-y-4">
        {CARS.map((car) => {
          ...
          // determine displayFare and meta (city packs include meta)
          const displayTotal = fare.total ?? 0;
          const packMeta = fare.meta || null;

          return (
            <div key={car.id} className="border rounded-xl overflow-hidden bg-white shadow-sm">
              <div className="p-4 flex items-center gap-4">
                <div className="flex-shrink-0">
                  <img src={car.img} alt={car.title} className="w-28 h-20 object-cover rounded-lg border" onError={(e) => handleImageError(e, car.id)} />
                </div>

                <div className="flex-1 min-w-0">
                  <h3 className="font-semibold text-gray-900">{car.title}</h3>
                  <p className="text-xs text-gray-600">{car.description} | {car.seats}</p>

                  {service === "airport" && airportCfg && (
                    <p className="text-sm text-gray-700 mt-1">
                      Base {airportCfg.baseKm}{airportCfg.baseIsHours ? " (4h/40km)" : "km"} • Extra ₹{airportCfg.extraPerKm}/km • After {airportCfg.waitFreeMin / (airportCfg.baseIsHours ? 60 : 1)}{airportCfg.baseIsHours ? "h" : "m"}: ₹{airportCfg.waitPerMin}/min
                    </p>
                  )}
                  {service === "local" && (
                    <p className="text-sm text-gray-700 mt-1">Includes base hours & kms • Extra km/hr as per car (Package: {packMeta?.packageKey || cityPackKey})</p>
                  )}
                  {service === "outstation" && searchParams?.tripType === "oneway" && (
                    <p className="text-sm text-gray-700 mt-1">Min billable 100km • Extra as per car</p>
                  )}
                  {service === "outstation" && searchParams?.tripType === "roundtrip" && (
                    <p className="text-sm text-gray-700 mt-1">Min {PRICE.roundtrip.dailyMinKm}km/day • ₹{PRICE.roundtrip.perKm[car.id]}/km • Driver ₹{premium(car.id) ? PRICE.roundtrip.driverAllowance.premium : PRICE.roundtrip.driverAllowance.default}/day</p>
                  )}

                  <button type="button" onClick={() => setOpenId(isOpen ? null : car.id)} className="mt-2 text-sm text-rose-600 hover:text-rose-700">
                    View Fare Detail {isOpen ? "▴" : "▾"}
                  </button>
                </div>

                <div className="text-right flex-shrink-0">
                  <div className="text-2xl font-bold">₹ {toFixedMoney(Math.round(displayTotal))}</div>
                  <button onClick={() => {
                    // send package info in selection payload
                    const payload = {
                      carId: car.id,
                      fare,
                      service,
                      ...
                      arrivalDelayMin,
                      days,
                      // include package details when local service
                    ...
                    className="mt-2 px-4 py-2 bg-sky-600 text-white rounded-md hover:bg-sky-700 text-sm font-medium">Book Now</button>
                  <div className="text-xs text-gray-500 mt-1">ETA: {prettyDuration(arrivalDelayMin)}</div>
                </div>
              </div>
                ...
                <div className="px-4 pb-4 text-sm text-gray-700 border-t bg-gray-50">
                  <div className="py-3">
                    {service === "airport" && (
                      <>
                        <div>
                          Base ₹{airportCfg.baseFare} for {airportCfg.baseKm}{airportCfg.baseIsHours ? " (4h/40km)" : "km"} • Extra km: {fare.extraKm} × ₹{airportCfg.extraPerKm} = ₹{(fare.distanceFare - airportCfg.baseFare).toLocaleString("en-IN")}
                        </div>
                        {fare.waitFare > 0 && (
                          <div>Waiting after free {airportCfg.waitFreeMin}{airportCfg.baseIsHours ? "min (4h)" : "min"}: ₹{fare.waitFare.toLocaleString("en-IN")}</div>
                        )}
                        <div className="mt-1">Tolls/Parking extra at actuals.</div>
                      </>
                    )}
                    {service === "outstation" && searchParams?.tripType === "oneway" && (
                      <>
                        <div>Billable km: {fare.effectiveKm} (min {PRICE.oneway[car.id].baseKm})</div>
                        {fare.extraKm > 0 && (
                          <div>Extra km: {fare.extraKm} × ₹{PRICE.oneway[car.id].extraPerKm} = ₹{(fare.extraKm * PRICE.oneway[car.id].extraPerKm).toLocaleString("en-IN")}</div>
                        )}
                      </>
                    )}
                    {service === "outstation" && searchParams?.tripType === "roundtrip" && (
                      <>
                        <div>Days: {fare.days} • Actual km: {km || 0} • Billable km: {fare.billableKm} (min {PRICE.roundtrip.dailyMinKm}×{fare.days})</div>
                        <div>Kilometer charges: {fare.billableKm} × ₹{fare.perKm} = ₹{fare.kmFare.toLocaleString("en-IN")}</div>
                        <div>Driver allowance: ₹{fare.daPerDay}/day × {fare.days} = ₹{fare.daTotal.toLocaleString("en-IN")}</div>
                      </>
                    )}
                    {service === "local" && (
                      <div>Billing from Pick-up to Pick-up • DA after 12am: ₹{PRICE.city.driverAllowanceAfterMidnight}</div>
                    )}
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="bg-white rounded-lg p-3 border">
                      ...
                      <ul className="mt-2 text-[13px] space-y-1 list-disc list-inside">
                        <li>Professional driver & clean A.C. cab</li>
                        <li>Base package / base km as per service selection</li>
                    ...
                    <div className="bg-white rounded-lg p-3 border">
                      ...
                      <ul className="mt-2 text-[13px] space-y-1 list-disc list-inside">
                        ...
                        <li>Night charges / Driver allowance when applicable</li>
                      </ul>
                    </div>
                  </div>
                </div>
